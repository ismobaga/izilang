...
std::string ErrorReporter::formatError(const Token& token, const std::string& message,
    ...
    // Error header
    oss << colorize(errorType, Color::RED) << colorize(" at line ", Color::BOLD)
        << token.line << colorize(", column ", Color::BOLD) << token.column << ":\n";
    ...
    if (!lineContent.empty()) {
        ...
        oss << "  " << std::string(lineNumStr.length(), ' ') << " | "
            << colorize(underline, Color::RED) << "\n";
    }
...
std::string ErrorReporter::formatError(int line, int column, const std::string& message,
    ...
    // Error header
    oss << colorize(errorType, Color::RED) << colorize(" at line ", Color::BOLD)
        << line << colorize(", column ", Color::BOLD) << column << ":\n";
